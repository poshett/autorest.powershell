...
import {
  ...
  JsonType,
  http,
  getAllPublicVirtualProperties,
  getVirtualPropertyFromPropertyName,
  ParameterLocation,
  getAllVirtualProperties,
  VirtualParameter,
  VirtualProperty,
...
import {
  CommandOperation,
  OperationType,
  VirtualParameter as NewVirtualParameter,
} from '../utils/command-operation';
import {
  getAllProperties as NewGetAllProperties,
  getAllPublicVirtualProperties as NewGetAllPublicVirtualProperties,
  getVirtualPropertyFromPropertyName as NewGetVirtualPropertyFromPropertyName,
  VirtualProperty as NewVirtualProperty,
} from '../utils/schema';
import {
  escapeString,
  docComment,
...
export function addCompleterInfo(
  targetProperty: Property,
  parameter: VirtualParameter
) {
  if (parameter.completerInfo && parameter.completerInfo.script) {
    targetProperty.add(
      new Attribute(ClientRuntime.CompleterInfoAttribute, {
        parameters: [
          new LiteralExpression(
            `\nName = ${
              new StringExpression(parameter.completerInfo.name || '').value
            }`
          ),
          new LiteralExpression(
            `\nDescription =${
              new StringExpression(parameter.completerInfo.description || '')
                .value
            }`
          ),
          new LiteralExpression(
            `\nScript = ${
              new StringExpression(parameter.completerInfo.script).value
            }`
          ),
        ],
      })
...
export function NewAddCompleterInfo(
  targetProperty: Property,
  parameter: NewVirtualParameter
) {
  if (parameter.completerInfo && parameter.completerInfo.script) {
    targetProperty.add(
      new Attribute(ClientRuntime.CompleterInfoAttribute, {
...
export function addParameterBreakingChange(
  targetProperty: Property,
  parameter: any
) {
  if (parameter.breakingChange) {
    ...
    if (parameter.breakingChange.replacement)
      parameters.push(
        `ReplaceMentCmdletParameterName="${parameter.breakingChange.replacement}"`
      );
    if (parameter.breakingChange.isBecomingMandatory)
      parameters.push(
        `IsBecomingMandatory=${parameter.breakingChange.isBecomingMandatory}`
      );
    if (parameter.breakingChange.changeDescription)
      parameters.push(
        `ChangeDescription="${parameter.breakingChange.changeDescription}"`
      );
    if (parameter.breakingChange.newParameterType) {
      ...
      if (parameter.breakingChange.oldParamaterType) {
        parameters.push(
          `OldParamaterType="${parameter.breakingChange.oldParamaterType}"`
        );
      } else {
        // for primitive types, should use name, otherwise use fullname which contains namespace
        parameters.push(
          `OldParamaterType="${
            parameter.schema.language.csharp.fullname.startsWith('<')
              ? parameter.schema.language.csharp.name
              : parameter.schema.language.csharp.fullname
          }"`
        );
      }
      parameters.push(
        `NewParameterType="${parameter.breakingChange.newParameterType}"`
      );
    }
    targetProperty.add(
      new Attribute(ClientRuntime.ParameterBreakingChangeAttribute, {
...
export function addPreviewMessage(targetProperty: Property, parameter: any) {
  if (parameter.previewMessage) {
    targetProperty.add(
      new Attribute(ClientRuntime.PreviewMessageAttribute, {
        parameters: [`"${parameter.previewMessage}"`],
      })
    );
  }
}
...
export function addDefaultInfo(targetProperty: Property, parameter: any) {
  if (parameter.defaultInfo && parameter.defaultInfo.script) {
    targetProperty.add(
      new Attribute(ClientRuntime.DefaultInfoAttribute, {
        parameters: [
          new LiteralExpression(
            `\nName = ${
              new StringExpression(parameter.defaultInfo.name || '').value
            }`
          ),
          new LiteralExpression(
            `\nDescription =${
              new StringExpression(parameter.defaultInfo.description || '')
                .value
            }`
          ),
          new LiteralExpression(
            `\nScript = ${
              new StringExpression(parameter.defaultInfo.script).value
            }`
          ),
          new LiteralExpression(
            `\nSetCondition = ${
              new StringExpression(parameter.defaultInfo['set-condition'] || '')
                .value
            }`
          ),
        ],
...
export function addInfoAttribute(
  targetProperty: Property,
    ...
    switch (pt.elementType.schema.type) {
      case JsonType.Object:
        if (pt.elementType.schema.details.csharp.interfaceImplementation) {
          pt = {
            declaration:
              pt.elementType.schema.details.csharp.interfaceImplementation
                .declaration,
  ...
  if (pt.schema && pt.schema && pt.schema.details.csharp.byReference) {
    ptypes.push(
      `typeof(${pt.schema.details.csharp.namespace}.${pt.schema.details.csharp.interfaceName}_Reference)`
    );
    ...
    if (
      pt.schema &&
      pt.schema.details.csharp.classImplementation &&
      pt.schema.details.csharp.classImplementation.discriminators
    ) {
      ptypes.push(
        ...[
          ...pt.schema.details.csharp.classImplementation.discriminators.values(),
        ].map((each) => `typeof(${each.modelInterface.fullName})`)
      );
    }
  }

  targetProperty.add(
    new Attribute(ClientRuntime.InfoAttribute, {
      parameters: [
        new LiteralExpression(`\nRequired = ${isRequired}`),
        new LiteralExpression(`\nReadOnly = ${isReadOnly}`),
        new LiteralExpression(
...
export function NewAddInfoAttribute(
  targetProperty: Property,
  pType: TypeDeclaration,
  isRequired: boolean,
  isReadOnly: boolean,
  ...
  if (pt.schema && pt.schema && pt.schema.language.csharp.byReference) {
    ptypes.push(
      `typeof(${pt.schema.language.csharp.namespace}.${pt.schema.language.csharp.interfaceName}_Reference)`
    );
    // do we need polymorphic types for by-resource ? Don't think so.
  } else {
    ptypes.push(`typeof(${pt.declaration})`);
    if (
      pt.schema &&
      pt.schema.language.csharp.classImplementation &&
      pt.schema.language.csharp.classImplementation.discriminators
    ) {
      ptypes.push(
        ...[
          ...pt.schema.language.csharp.classImplementation.discriminators.values(),
        ].map((each) => `typeof(${each.modelInterface.fullName})`)
      );
    }
  }
...
export class CmdletClass extends Class {
  ...
  constructor(
    ...
    state: State,
    objectInitializer?: DeepPartial<CmdletClass>
  ) {
    ...
    const variantName = `${noun}${
      operation.details.csharp.name ? `_${operation.details.csharp.name}` : ''
    }`;

    const name = `${operation.details.csharp.verb}${variantName}`;
    super(namespace, name, PSCmdlet);
    this.dropBodyParameter = operation.details.csharp.dropBodyParameter
      ? true
      : false;
    ...
    this.eventListener = new EventListener(
      new LiteralExpression(`((${ClientRuntime.IEventListener})this)`),
      true
    );

    this.isViaIdentity = variantName.indexOf('ViaIdentity') > 0;
    this.clientsidePagination =
      !!operation.details.csharp.clientsidePagination &&
      !!operation.callGraph[0].language.csharp?.pageable;
  }
  ...
  async init() {
    ...
    this.add(
      new Method('BeginProcessing', dotnet.Void, {
        ...
        *body() {
          if ($this.state.project.azure) {
            ...
            yield If(
              'telemetryId != "" && telemetryId != "internal"',
          ...
          yield If(
            $this.$<Property>('Break'),
            `${ClientRuntime.AttachDebugger}.Break();`
          );
    ...
    if (this.hasStreamOutput) {
      this.outFileParameter = this.add(
        new Property('OutFile', System.String, {
          attributes: [],
          description: 'Path to write output file to.',
        })
      );
      this.outFileParameter.add(
        new Attribute(ParameterAttribute, {
          parameters: [
            'Mandatory = true',
            'HelpMessage = "Path to write output file to"',
          ],
        })
      );
  ...
  public get headerComment(): string {
    ...
    for (const httpOperation of values(this.operation.callGraph)) {
      ...
      ops = `${ops}\n[OpenAPI] ${
        httpOperation.language.default.name
      }=>${httpMethod.toUpperCase()}:"${httpPath}"`;
      if (this.debugMode) {
  ...
  private addCommonStuff() {
    ...
    const privateInvocationInfo = this.add(
      new Field('__invocationInfo', InvocationInfo, {
        description:
          'A copy of the Invocation Info (necessary to allow asJob to clone this cmdlet)',
        access: Access.Private,
      })
    );
    this.invocationInfo = new Property(
      'InvocationInformation',
      InvocationInfo,
      { description: 'Accessor for our copy of the InvocationInfo.' }
    );
    this.invocationInfo.get = toExpression(
      `${privateInvocationInfo.value} = ${privateInvocationInfo.value} ?? this.MyInvocation `
    );
    this.invocationInfo.set = new Statements(
      privateInvocationInfo.assign('value')
    );
    this.add(this.invocationInfo);

    if (this.state.project.azure) {
      this.correlationId = this.add(
        new Field('__correlationId', dotnet.String, {
          initialValue: 'System.Guid.NewGuid().ToString()',
          description:
            'A unique id generatd for the this cmdlet when it is instantiated.',
          access: Access.Private,
        })
      );
      this.processRecordId = this.add(
        new Field('__processRecordId', dotnet.String, {
          description:
    ...
    // pipeline property
    this.add(
      new Property('Pipeline', ClientRuntime.HttpPipeline, {
        getAccess: Access.Public,
        setAccess: Access.Public,
        description: `The instance of the <see cref="${ClientRuntime.HttpPipeline}" /> that the remote call will use.`,
      })
    );

    // client API property (gs01: fill this in correctly)
    const clientAPI = new ClassType(
      this.state.model.language.csharp?.namespace,
      this.state.model.language.csharp?.name || ''
    );
    this.add(
      new LambdaProperty(
        'Client',
        clientAPI,
        new LiteralExpression(
          `${this.state.project.serviceNamespace.moduleClass.declaration}.Instance.ClientAPI`
        ),
        { description: 'The reference to the client API class.' }
      )
    );

    this.add(
      new Method('StopProcessing', dotnet.Void, {
        access: Access.Protected,
        override: Modifier.Override,
        description: 'Interrupts currently running code within the command.',
      })
    ).add(function* () {
    ...
    const $this = this;
    this.add(
      new Method('EndProcessing', dotnet.Void, {
        access: Access.Protected,
        override: Modifier.Override,
        description: 'Performs clean-up after the command execution',
      })
    ).add(function* () {
    ...
    // debugging
    const brk = this.add(
      new Property('Break', SwitchParameter, {
        attributes: [],
        description: 'Wait for .NET debugger to attach',
      })
    );
    brk.add(
      new Attribute(ParameterAttribute, {
    ...
    const prepend = this.add(
      new Property('HttpPipelinePrepend', ClientRuntime.SendAsyncSteps, {
        attributes: [],
        description:
    ...
    const append = this.add(
      new Property('HttpPipelineAppend', ClientRuntime.SendAsyncSteps, {
        attributes: [],
        description:
    ...
    const proxyCredential = this.add(
      new Property('ProxyCredential', PSCredential, {
        attributes: [],
        description:
    ...
    const useDefaultCreds = this.add(
      new Property('ProxyUseDefaultCredentials ', SwitchParameter, {
        attributes: [],
        description: 'Use the default credentials for the proxy',
      })
    );
    useDefaultCreds.add(
      new Attribute(ParameterAttribute, {
    ...
    const proxyUri = this.add(
      new Property('Proxy', System.Uri, {
        attributes: [],
        description: 'The URI for the proxy server to use',
      })
    );
    proxyUri.add(
      new Attribute(ParameterAttribute, {
    ...
    if (this.state.project.azure) {
      this.defaultProfile = this.add(
        new Property('DefaultProfile', PSObject, {
          description:
            'The credentials, account, tenant, and subscription used for communication with Azure',
        })
      );
      this.defaultProfile.add(
        new Attribute(ParameterAttribute, {
          parameters: [
            'Mandatory = false',
            'HelpMessage = "The credentials, account, tenant, and subscription used for communication with Azure."',
          ],
        })
      );
      ...
      this.defaultProfile.add(
        new Attribute(Alias, {
          parameters: ['"AzureRMContext"', '"AzureCredential"'],
        })
      );
      this.defaultProfile.add(
        new Attribute(CategoryAttribute, {
          parameters: [`${ParameterCategory}.Azure`],
        })
      );
    }
  }
  ...
  private NewIsWritableCmdlet(operation: CommandOperation): boolean {
    if (operation.callGraph[0].requests) {
      switch (
        operation.callGraph[0].requests[0]?.protocol.http?.method.toLowerCase()
      ) {
        case 'put':
        case 'post':
        case 'delete':
  ...
  private NewImplementProcessRecord(operation: CommandOperation) {
    ...
    this.add(
      new Method('ProcessRecord', undefined, {
        access: Access.Protected,
        override: Modifier.Override,
      ...
      yield Try(function* () {
        ...
        const normal = new Statements(function* () {
          const acr = new LocalVariable('asyncCommandRuntime', dotnet.Var, {
            initializer: AsyncCommandRuntime.new(
              dotnet.This,
        ...
        if (operation.asjob) {
          const asjob = $this.add(
            new Property('AsJob', SwitchParameter, {
              description:
          ...
          asjob.add(
            new Attribute(ParameterAttribute, {
              parameters: [
                'Mandatory = false',
                'HelpMessage = "Run the command as a job"',
              ],
            })
          );
          asjob.add(
          ...
          const nowait = $this.add(
            new Property('NoWait', SwitchParameter, {
              description:
                'when specified, will make the remote call, and return an AsyncOperationResponse, letting the remote operation continue asynchronously.',
            })
          );
          nowait.add(
            new Attribute(ParameterAttribute, {
              parameters: [
                'Mandatory = false',
                'HelpMessage = "Run the command asynchronously"',
              ],
            })
          );
          nowait.add(
        ...
        const work: OneOrMoreStatements = operation.asjob
          ? function* () {
            yield If(
              'true == MyInvocation?.BoundParameters?.ContainsKey("AsJob")',
              function* () {
                // clone the cmdlet instance, since the instance can be reused and overwrite data.
                const instance = new LocalVariable('instance', dotnet.Var, {
                  initializer: 'this.Clone()',
                });
                yield instance.declarationStatement;

                // create the job (which will set the CommandRuntime of the clone to the AsyncJob itself)
                const job = new LocalVariable('job', dotnet.Var, {
                  initializer: AsyncJob.new(
                    instance,
                    'this.MyInvocation.Line, this.MyInvocation.MyCommand.Name, this._cancellationTokenSource.Token',
                ...
                const task = new LocalVariable('task', dotnet.Var, {
                  initializer: `${instance}.ProcessRecordAsync()`,
                });
        ...
        if (writable) {
          yield If(
            `ShouldProcess($"Call remote '${operation.callGraph[0].language.csharp?.name}' operation")`,
            work
          );
        } else {
          yield work;
      ...
      const aggregateException = new Parameter(
        'aggregateException',
      ...
      yield Catch(aggregateException, function* () {
        ...
        yield ForEach(
          'innerException',
          new LiteralExpression(
            `${aggregateException.use}.Flatten().InnerExceptions`
          ),
          function* () {
            yield $this.eventListener.syncSignal(
              Events.CmdletException,
              new LiteralExpression(
                '$"{innerException.GetType().Name} - {innerException.Message} : {innerException.StackTrace}"'
              )
            );
            yield '// Write exception out to error channel.';
            yield `WriteError( new ${ErrorRecord}(innerException,string.Empty, ${ErrorCategory(
              'NotSpecified'
            )}, null) );`;
          }
        );
      ...
      yield Catch(
        ...
        function* () {
          yield $this.eventListener.syncSignal(
            Events.CmdletException,
            new LiteralExpression(
              `$"{${exception.use}.GetType().Name} - {${exception.use}.Message} : {${exception.use}.StackTrace}"`
            )
          );
          yield '// Write exception out to error channel.';
          yield `WriteError( new ${ErrorRecord}(${
            exception.use
          },string.Empty, ${ErrorCategory('NotSpecified')}, null) );`;
        },
        {
          when: new LiteralExpression(
            '(exception as System.Management.Automation.PipelineStoppedException)== null || (exception as System.Management.Automation.PipelineStoppedException).InnerException != null'
          ),
        }
      );

      yield Finally(function* () {
        yield $this.eventListener.syncSignalNoCheck(
  ...
  private NewImplementProcessRecordAsync(operation: CommandOperation) {
    const $this = this;
    const PAR = this.add(
      new Method('ProcessRecordAsync', System.Threading.Tasks.Task(), {
        access: Access.Protected,
    ...
    PAR.add(function* () {
      ...
      if ($this.state.project.azure) {
        yield pipeline.assign(
          new LiteralExpression(
            `${$this.state.project.serviceNamespace.moduleClass.declaration}.Instance.CreatePipeline(${$this.invocationInfo}, ${$this.correlationId}, ${$this.processRecordId}, this.ParameterSetName, this.ExtensibleParameters)`
          )
        );
      } else {
        yield pipeline.assign(
          new LiteralExpression(
            `${$this.state.project.serviceNamespace.moduleClass.declaration}.Instance.CreatePipeline(${$this.invocationInfo}, this.ParameterSetName, this.ExtensibleParameters)`
          )
        );
      }

      yield If(
        IsNotNull($this.$<Property>('HttpPipelinePrepend')),
        pipeline.invokeMethod(
          'Prepend',
          toExpression(
            `(this.CommandRuntime as Microsoft.Rest.ClientRuntime.PowerShell.IAsyncCommandRuntimeExtensions)?.Wrap(${$this.$<Property>(
              'HttpPipelinePrepend'
            )}) ?? ${$this.$<Property>('HttpPipelinePrepend')}`
          )
        )
      );
      yield If(
        IsNotNull($this.$<Property>('HttpPipelineAppend')),
        pipeline.invokeMethod(
          'Append',
          toExpression(
            `(this.CommandRuntime as Microsoft.Rest.ClientRuntime.PowerShell.IAsyncCommandRuntimeExtensions)?.Wrap(${$this.$<Property>(
              'HttpPipelineAppend'
            )}) ?? ${$this.$<Property>('HttpPipelineAppend')}`
          )
        )
      );
      ...
      if ($this.operation.extensions.inputPipe) {
        const handlers = <Array<HandlerDirective>>(
          $this.operation.extensions.inputPipe
        );
        // default priority is 100
        handlers.sort((a, b) => {
          return (a.priority || 100) - (b.priority || 100);
        });
        for (const { index, handler } of handlers.map((handler, index) => ({
          index,
          handler,
        }))) {
        ...
        if (vParam.hidden) {
          const td =
            $this.state.project.schemaDefinitionResolver.resolveTypeDeclaration(
              <NewSchema>vParam.schema,
              true,
              $this.state
            );
          yield If(
            `true == this.MyInvocation?.BoundParameters?.ContainsKey("${vParam.name}")`,
            `${vParam.name} = (${td.declaration})this.MyInvocation.BoundParameters["${vParam.name}"];`
          );
        }
      }
      ...
      const operationParameters = values(apiCall.parameters)
        // filter out constants and path parameters when using piping for identity
        .where(
          (each) =>
            !each.language.csharp?.constantValue &&
            each.language.default?.name !==
              '$host' /* && (!$this.isViaIdentity || each.in !== ParameterLocation.Path) */
        )
        .select((p) => {
          return {
            ...
            param: values($this.properties)
              ...
              .first(
                (each) =>
                  each.metadata.parameterDefinition.language.csharp
                    ?.serializedName === p.language.csharp?.serializedName
              ), // xichen: Is it safe enough to use serializedName?
            isPathParam:
              $this.isViaIdentity &&
        ...
        .select((each) => {
          if (each.param) {
            const httpParam = <HttpParameter>(
              each.param.metadata.parameterDefinition
            );
            ...
            return {
              ...
              expression: toExpression(
                `this.InvocationInformation.BoundParameters.ContainsKey("${each.param.value}") ? ${each.param.value} : ${httpParamTD.defaultOfType}`
              ),
              isPathParam: each.isPathParam,
            };
          }
      ...
      const responses = [
        ...values(apiCall.responses),
        ...values(apiCall.exceptions),
      ];

      const callbackMethods = values(responses)
        .toArray()
        .map((each) => new LiteralExpression(each.language.csharp?.name || ''));
      ...
      for (const each of values(responses)) {
        ...
        parameters.push(
          new Parameter(
            ...
            {
              description: `the raw response message as an ${System.Net.Http.HttpResponseMessage}.`,
            }
          )
        );
          ...
          parameters.push(
            new Parameter(
              ...
              System.Threading.Tasks.Task({
                declaration: each.language.csharp?.responseType,
              }),
              {
                description: `the body result as a <see cref="${each.language.csharp?.responseType.replace(
                  /\[|\]|\?/g,
                  ''
                )}">${
                  each.language.csharp?.responseType
                }</see> from the remote call`,
              }
            )
          );
        }
        if (each.language.csharp?.headerType) {
          parameters.push(
            new Parameter(
              'headers',
              System.Threading.Tasks.Task({
                declaration: each.language.csharp.headerType,
              }),
              {
                description: `the header result as a <see cref="${each.language.csharp.headerType}" /> from the remote call`,
              }
            )
          );
        }
        ...
        if (isBinary) {
          parameters.push(
            new Parameter(
              'response',
              System.Threading.Tasks.Task({
                declaration: 'global::System.IO.Stream',
              }),
              {
                description:
                  'the body result as a <see cref="global::System.IO.Stream" /> from the remote call',
              }
            )
          );
        }

        const override = `override${pascalCase(
          each.language.csharp?.name || ''
        )}`;
        const returnNow = new Parameter(
          'returnNow',
          System.Threading.Tasks.Task(dotnet.Bool),
          {
            modifier: ParameterModifier.Ref,
            description: `/// Determines if the rest of the ${each.language.csharp?.name} method should be processed, or if the method should return immediately (set to true to skip further processing )`,
          }
        );
        const overrideResponseMethod = new PartialMethod(
          override,
          dotnet.Void,
          {
        ...
        const responseMethod = new Method(
          `${each.language.csharp?.name}`,
          System.Threading.Tasks.Task(),
          {
            access: Access.Private,
        ...
        responseMethod.add(function* () {
          const skip = Local(
            '_returnNow',
            `${
              System.Threading.Tasks.Task(dotnet.Bool).declaration
            }.FromResult(${dotnet.False})`
          );
          yield skip.declarationStatement;
          yield `${overrideResponseMethod.invoke(
            ...parameters,
            `ref ${skip.value}`
          )};`;
          ...
          if (each.language.csharp?.isErrorResponse) {
            ...
            const unexpected = function* () {
              ...
              const ex = each.language.csharp?.responseType
                ? Local(
                  'ex',
                  `new ${ClientRuntime.name}.RestException<${each.language.csharp.responseType}>(responseMessage, await response)`
                )
                : Local(
                  'ex',
                  `new ${ClientRuntime.name}.RestException(responseMessage)`
                );

              yield ex.declarationStatement;

              yield `WriteError( new global::System.Management.Automation.ErrorRecord(${
                ex.value
              }, ${
                ex.value
              }.Code, global::System.Management.Automation.ErrorCategory.InvalidOperation, new { ${operationParameters
                .filter((e) => valueOf(e.expression) !== 'null')
                .map((each) => `${each.name}=${each.expression}`)
                .join(', ')} })
{
  ErrorDetails = new global::System.Management.Automation.ErrorDetails(${
  ex.value
}.Message) { RecommendedAction = ${ex.value}.Action }
});`;
            };
            if ((<SchemaResponse>each).schema !== undefined) {
              ...
              let props = NewGetAllPublicVirtualProperties(
                (<SchemaResponse>each).schema.language.csharp?.virtualProperties
              );
              const errorProperty = values(props).first(
                (p) => p.property.serializedName === 'error'
              );
              let ep = '';
              if (errorProperty) {
                props = NewGetAllPublicVirtualProperties(
                  errorProperty.property.schema.language.csharp
                    ?.virtualProperties
                );
              ...
              const codeProp = props.find(
                (p) => p.name.toLowerCase().indexOf('code') > -1
              ); // first property with 'code'
              const messageProp = props.find(
                (p) => p.name.toLowerCase().indexOf('message') > -1
              ); // first property with 'message'
              const actionProp = props.find(
                (p) => p.name.toLowerCase().indexOf('action') > -1
              ); // first property with 'action'

              if (codeProp && messageProp) {
                const lcode = new LocalVariable('code', dotnet.Var, {
                  initializer: `(await response)?.${ep}${codeProp.name}`,
                });
                const lmessage = new LocalVariable('message', dotnet.Var, {
                  initializer: `(await response)?.${ep}${messageProp.name}`,
                });
                const laction = actionProp
                  ? new LocalVariable('action', dotnet.Var, {
                    initializer: `(await response)?.${ep}${actionProp.name} ?? ${System.String.Empty}`,
                  })
                  : undefined;
                yield lcode;
                yield lmessage;
                ...
                yield Else(`WriteError( new global::System.Management.Automation.ErrorRecord(new global::System.Exception($"[{${lcode}}] : {${lmessage}}"), ${lcode}?.ToString(), global::System.Management.Automation.ErrorCategory.InvalidOperation, new { ${operationParameters
                  .filter((e) => valueOf(e.expression) !== 'null')
                  .map((each) => `${each.name}=${each.expression}`)
                  .join(', ')} })
{
  ErrorDetails = new global::System.Management.Automation.ErrorDetails(${lmessage}) { RecommendedAction = ${
  laction || System.String.Empty
} }
});`);
                return;
              } else {
          ...
          yield `// ${
            each.language.csharp?.name
          } - response for ${each.protocol.http?.statusCodes[0]} / ${values(each.protocol.http?.mediaTypes).join('/')}`;
          ...
          if ('schema' in each) {
            ...
            if (apiCall.language.csharp?.pageable) {
              ...
              yield `// ${pageable.responseType} / ${
                pageable.itemName || '<none>'
              } / ${pageable.nextLinkName || '<none>'}`;
              switch (pageable.responseType) {
                // the result is (or works like a x-ms-pageable)
                case 'pageable':
                case 'nested-array':
                  {
                    const valueProperty = (<ObjectSchema>(
                      schema
                    )).properties?.find(
                      (p) => p.serializedName === pageable.itemName
                    );
                    const nextLinkProperty = (<ObjectSchema>(
                      schema
                    ))?.properties?.find(
                      (p) => p.serializedName === pageable.nextLinkName
                    );
                    if (valueProperty && nextLinkProperty) {
                      // it's pageable!
                      const result = new LocalVariable('result', dotnet.Var, {
                        initializer: new LiteralExpression('await response'),
                      });
                      yield result.declarationStatement;
                      // write out the current contents
                      const vp = NewGetVirtualPropertyFromPropertyName(
                        schema.language.csharp?.virtualProperties,
                        valueProperty.serializedName
                      );
                      if (vp) {
                        if ($this.clientsidePagination) {
                          yield If(
                            '(ulong)result.Value.Length <= this.PagingParameters.Skip',
                            function* () {
                      ...
                      const nl = NewGetVirtualPropertyFromPropertyName(
                        schema.language.csharp?.virtualProperties,
                        nextLinkProperty.serializedName
                      );
                      if (nl) {
                        $this.add(
                          new Field('_isFirst', dotnet.Bool, {
                            access: Access.Private,
                        ...
                        $this.add(
                          new Field('_nextLink', dotnet.String, {
                            access: Access.Private,
                        ...
                        const nextLinkCondition = $this.clientsidePagination
                          ? '_nextLink != null && this.PagingParameters.First > 0'
                          : '_nextLink != null';
                        yield If('_isFirst', function* () {
                          ...
                          yield While(
                            nextLinkCondition,
                            If(
                              'responseMessage.RequestMessage is System.Net.Http.HttpRequestMessage requestMessage ',
                              function* () {
                                yield `requestMessage = requestMessage.Clone(new global::System.Uri( _nextLink ),${ClientRuntime.Method.Get} );`;
                                yield $this.eventListener.signal(
                                  Events.FollowingNextLink
                                );
                                yield `await this.${
                                  $this
                                    .$<Property>('Client')
                                    .invokeMethod(
                                      `${apiCall.language.csharp?.name}_Call`,
                                      ...[
                                        toExpression('requestMessage'),
                                        ...callbackMethods,
                                        dotnet.This,
                                        pipeline,
                                      ]
                      ...
                      const p = getVirtualPropertyFromPropertyName(
                        schema.language.csharp?.virtualProperties,
                        valueProperty.serializedName
                      );
                      if (p) {
            ...
            const props = NewGetAllPublicVirtualProperties(
              schema.language.csharp?.virtualProperties
            );
            const outValue =
              length(props) === 1
                ? `(await response).${props[0].name}`
                : '(await response)';
            ...
            if ($this.hasStreamOutput && $this.outFileParameter) {
              ...
              const paths = Local(
                'paths',
                `this.SessionState.Path.GetResolvedProviderPathFromPSPath(${outfile.value}, out ${provider.declarationExpression})`
              );
              yield paths.declarationStatement;
              yield If(
                `${provider.value}.Name != "FileSystem" || ${paths.value}.Count == 0`,
                `ThrowTerminatingError( new System.Management.Automation.ErrorRecord(new global::System.Exception("Invalid output path."),string.Empty, global::System.Management.Automation.ErrorCategory.InvalidArgument, ${outfile.value}) );`
              );
              yield If(
                `${paths.value}.Count > 1`,
                `ThrowTerminatingError( new System.Management.Automation.ErrorRecord(new global::System.Exception("Multiple output paths not allowed."),string.Empty, global::System.Management.Automation.ErrorCategory.InvalidArgument, ${outfile.value}) );`
              );
              ...
              if (rType.declaration === System.IO.Stream.declaration) {
                ...
                yield Using(stream.declarationExpression, function* () {
                  const fileStream = Local(
                    'fileStream',
                    `global::System.IO.File.OpenWrite(${paths.value}[0])`
                  );
                  yield Using(
                    fileStream.declarationExpression,
                    `await ${stream.value}.CopyToAsync(${fileStream.value});`
                  );
                });
              } else {
              ...
              yield If(
                'true == MyInvocation?.BoundParameters?.ContainsKey("PassThru")',
                function* () {
          ...
          if ('binary' in each) {
            ...
            if ($this.hasStreamOutput && $this.outFileParameter) {
              ...
              const paths = Local(
                'paths',
                'new global::System.Collections.ObjectModel.Collection<global::System.String>()'
              );
              yield paths.declarationStatement;
              yield Try(function* () {
              ...
              const notfound = new Parameter('', {
                declaration:
                  'global::System.Management.Automation.ItemNotFoundException',
              });
      ...
      yield Try(function* () {
        ...
        const actualCall = function* () {
          ...
          const idOpParams = operationParameters.filter(
            (each) => !each.isPathParam
          );
          const idschema = values(
            $this.state.project.model.schemas.objects
          ).first(
            (each) => each.language.default.uid === 'universal-parameter-type'
          );
          ...
          if ($this.isViaIdentity) {
            const identityFromPathParams = function* () {
              ...
              if (idschema) {
                const allVPs = NewGetAllPublicVirtualProperties(
                  idschema.language.csharp?.virtualProperties
                );
                ...
                const idOpParams = operationParameters.map((each) => {
                  ...
                  const match = props.find(
                    (p) => pascalCase(p.serializedName) === pascalName
                  );
                  if (match) {
                    const defaultOfType =
                      $this.state.project.schemaDefinitionResolver.resolveTypeDeclaration(
                    ...
                    // match up vp name
                    const vp = allVPs.find(
                      (pp) =>
                        pascalCase(pp.property.serializedName) === pascalName
                    );
                    if (vp) {
                      return {
                    ...
                    console.error(
                      `Unable to match identity parameter '${
                        each.name
                      }' member to appropriate virtual parameter. (Guessing '${pascalCase(
                        match.language.csharp?.name ?? ''
                      )}').`
                    );
                    return {
                      name: `InputObject.${pascalCase(
                        match.language.csharp?.name ?? ''
                      )}`,
                      value: `InputObject.${pascalCase(
                        match.language.csharp?.name ?? ''
                      )} ?? ${defaultOfType}`,
                    };
                  }
                  console.error(
                    `Unable to match idenity parameter '${each.name}' member to appropriate virtual parameter. (Guessing '${pascalName}')`
                  );
                  return {
                ...
                for (const opParam of idOpParams) {
                  if (opParam.name) {
                    yield If(
                      IsNull(opParam.name),
                      `ThrowTerminatingError( new ${ErrorRecord}(new global::System.Exception("InputObject has null value for ${
                        opParam.name
                      }"),string.Empty, ${ErrorCategory(
                        'InvalidArgument'
                      )}, InputObject) );`
                    );
                  }
                ...
                yield `await this.${
                  $this
                    .$<Property>('Client')
                    .invokeMethod(
                      `${apiCall.language.csharp?.name}`,
                      ...[
                        ...idOpParams.map((each) => toExpression(each.value)),
                        ...callbackMethods,
                        dotnet.This,
                        pipeline,
                      ]
            ...
            if (
              idschema &&
              values(idschema.properties).first(
                (each) =>
                  each.language.csharp?.uid ===
                  'universal-parameter:resource identity'
              )
            ) {
              const parameters = [
                toExpression('InputObject.Id'),
                ...idOpParams.map((each) => each.expression),
                ...callbackMethods,
                dotnet.This,
                pipeline,
              ];
              ...
              yield If(
                ...
                `await this.${
                  $this
                    .$<Property>('Client')
                    .invokeMethod(
                      `${apiCall.language.csharp?.name}ViaIdentity`,
                      ...parameters
                    ).implementation
                }`
              );
            ...
            const parameters = [
              ...operationParameters.map((each) => each.expression),
              ...callbackMethods,
              dotnet.This,
              pipeline,
            ];
            ...
            yield `await this.${
              $this
                .$<Property>('Client')
                .invokeMethod(`${apiCall.language.csharp?.name}`, ...parameters)
                .implementation
            }`;
          }
        ...
        if (
          ...
          operationParameters.find(
            (each) =>
              each.expression && each.expression.value === 'SubscriptionId'
          ) &&
          $this.operation.details.csharp.verb.toLowerCase() === 'get'
        ) {
      ...
      const ure = new Parameter('urexception', {
        declaration: `${ClientRuntime.fullName}.UndeclaredResponseException`,
      });
      yield Catch(ure, function* () {
        yield `WriteError(new global::System.Management.Automation.ErrorRecord(${
          ure.value
        }, ${ure.value}.StatusCode.ToString(), global::System.Management.Automation.ErrorCategory.InvalidOperation, new {  ${operationParameters
          .filter((e) => valueOf(e.expression) !== 'null')
          .map((each) => `${each.name}=${each.expression}`)
          .join(',')}})
{
  ErrorDetails = new global::System.Management.Automation.ErrorDetails(${
  ure.value
}.Message) { RecommendedAction = ${ure.value}.Action }
});`;
      });
      yield Finally(function* () {
        yield $this.eventListener.signalNoCheck(
          Events.CmdletProcessRecordAsyncEnd
        );
      });
    });
  }
  ...
  private implementConstructors() {
    ...
    this.add(
      new Constructor(this, {
        description: `Intializes a new instance of the <see cref="${this.name}" /> cmdlet class.`,
      })
    );
  }

  private implementIContext() {
    const extensibleParameters = this.add(
      new Field(
        '_extensibleParameters',
        System.Collections.Generic.Dictionary(System.String, System.Object),
        {
          access: Access.Private,
          initialValue: new LiteralExpression(
    ...
    const extensibleParametersProp = new Property(
      'ExtensibleParameters',
      System.Collections.Generic.IDictionary(System.String, System.Object),
      { description: 'Accessor for extensibleParameters.' }
    );
    extensibleParametersProp.get = toExpression(
      `${extensibleParameters.value} `
    );
  ...
  private implementIEventListener() {
    ...
    const cts = this.add(
      new Field(
        ...
        System.Threading.CancellationTokenSource,
        {
          access: Access.Private,
          initialValue: new LiteralExpression(
            `new ${System.Threading.CancellationTokenSource.declaration}()`
          ),
          description: `The <see cref="${System.Threading.CancellationTokenSource}" /> for this operation.`,
        }
      )
    );
    const cancellationTokenSource = new Property(
      'CancellationTokenSource',
      System.Threading.CancellationTokenSource,
      { description: 'Accessor for cancellationTokenSource.' }
    );
    ...
    this.add(
      new LambdaProperty(
        `${ClientRuntime.IEventListener}.Token`,
        System.Threading.CancellationToken,
        new LiteralExpression(`${cts.value}.Token`),
        {
          getAccess: Access.Default,
          setAccess: Access.Default,
          description: `<see cref="${ClientRuntime}.IEventListener" /> cancellation token.`,
        }
      )
    );
    this.cancellationToken = toExpression(
      `((${ClientRuntime.IEventListener})this).Token`
    );
    this.add(
      new LambdaProperty(
        `${ClientRuntime.IEventListener}.Cancel`,
        System.Action(),
        new LiteralExpression(`${cts.value}.Cancel`),
        {
          getAccess: Access.Default,
          setAccess: Access.Default,
          description: `<see cref="${ClientRuntime}.IEventListener" /> cancellation delegate. Stops the cmdlet when called.`,
        }
      )
    );
    ...
    const token = new Parameter('token', System.Threading.CancellationToken, {
      description:
        'The message cancellation token. When this call is cancelled, this should be <c>true</c>',
    });
    const messageData = new Parameter(
      'messageData',
      System.Func(ClientRuntime.EventData),
      { description: 'Detailed message data for the message event.' }
    );
    const signalMethod = this.add(
      new Method(
        `${ClientRuntime.IEventListener}.Signal`,
        System.Threading.Tasks.Task(),
        {
          async: Modifier.Async,
    ...
    signalMethod.add(function* () {
      ...
      const sw = Switch(id, [
        ...
        TerminalCase(Events.Information.value, function* () {
          if ($this.operation.asjob) {
            ...
            const data = new LocalVariable('data', dotnet.Var, {
              initializer: new LiteralExpression(`${messageData.use}()`),
            });
      ...
      if ($this.operation.asjob) {
        ...
        sw.add(
          Case(Events.DelayBeforePolling.value, function* () {
            yield 'var data = messageData();';
            yield If(
              'true == MyInvocation?.BoundParameters?.ContainsKey("NoWait")',
              function* () {
  ...
  private NewAddPowershellParameters(operation: CommandOperation) {
    ...
    for (const parameter of values(operation.parameters)) {
      ...
      if (parameter.details.csharp.fromHost) {
        ...
        const hostParameter = this.add(
          new BackedProperty(parameter.details.csharp.name, td, {
            metadata: {
        ...
        this.$<Method>('BeginProcessing').add(
          hostParameter.assignPrivate(
            new LiteralExpression(
              `${this.state.project.serviceNamespace.moduleClass.declaration}.Instance.GetParameter(this.MyInvocation, ${this.correlationId.value}, "${parameter.name}") as string`
            )
          )
        );
        continue;
      ...
      if (parameter.details.csharp.apiversion) {
        ...
        this.add(
          new Property(parameter.details.csharp.name, td, {
            ...
            *get() {
              ...
              const profiles =
                <Dictionary<string>>metadata.profiles ||
                new Dictionary<string>();

              yield Switch(
                `${$this.state.project.serviceNamespace.moduleClass.declaration}.Instance.Profile`,
                function* () {
                  for (const { key: profileName, value: apiVersion } of items(
                    profiles
                  )) {
                    yield TerminalCase(
                      `"${profileName}"`,
                  ...
                  yield TerminalDefaultCase(
                    Return(`"${metadata.apiVersions[0]}"`)
                  );
                }
              );
            },
      ...
      if (this.dropBodyParameter && parameter.details.csharp.isBodyParameter) {
        ...
        const expandedBodyParameter = this.add(
          new Field('_' + camelCase(parameter.details.csharp.name), td, {
            description: parameter.details.csharp.description,
            initialValue:
              parameter.schema.type === SchemaType.Array
                ? dotnet.Null
                : `new ${parameter.schema.language.csharp?.fullname}()`,
            access: Access.Private,
          })
        );
        ...
        for (const vParam of vps.body) {
          ...
          const nullable =
            this.state.project.schemaDefinitionResolver.resolveTypeDeclaration(
              vSchema,
              !!(<NewVirtualProperty>vParam.origin).required,
              this.state
            ).isNullable;

          const cmdletParameter = new Property(vParam.name, propertyType, {
            get: toExpression(
              `${expandedBodyParameter.value}.${
                getVirtualPropertyName(<any>vParam.origin) || vParam.origin.name
              }${!nullable ? '' : ` ?? ${propertyType.defaultOfType}`}`
            ), // /* ${inspect(vParam.origin)} */
            ...
            new: PropertiesRequiringNew.has(vParam.name)
              ? Modifier.New
              : Modifier.None,
          });
          ...
          if (vParam.schema.language.csharp?.byReference) {
            ...
            cmdletParameter.add(
              new Attribute(ExportAsAttribute, {
                parameters: [
                  `typeof(${vParam.schema.language.csharp.referenceInterface})`,
                ],
              })
            );
          }
          ...
          const dictSchema =
            vSchema.type === SchemaType.Dictionary
              ? vSchema
              : vSchema.type === SchemaType.Object
                ? (<ObjectSchema>vSchema).parents?.immediate?.find(
                  (s) => s.type === SchemaType.Dictionary
                )
                : undefined;
          if (dictSchema) {
            ...
            if (length((<ObjectSchema>vSchema).properties) === 0) {
              ...
              cmdletParameter.add(
                new Attribute(ExportAsAttribute, {
                  parameters: [`typeof(${System.Collections.Hashtable})`],
                })
              );
            } else {
          ...
          if (cmdletParameter.type.declaration === dotnet.Binary.declaration) {
            ...
            const ifname =
              vParam.name.toLowerCase() === 'value'
                ? 'InputFile'
                : pascalCase([vParam.name, 'Input', 'File']);
            ...
            const inputFileParameter = new Property(ifname, dotnet.String, {
              ...
              set: function* () {
                ...
                const paths = Local(
                  'paths',
                  `this.SessionState.Path.GetResolvedProviderPathFromPSPath(value, out ${provider.declarationExpression})`
                );
                yield paths.declarationStatement;
                yield If(
                  `${provider.value}.Name != "FileSystem" || ${paths.value}.Count == 0`,
                ...
                yield cmdletParameter.assign(
                  `global::System.IO.File.ReadAllBytes(${paths.value}[0])`
                );
              },
              description: `Input File for ${
                cmdletParameter.name
              } (${escapeString(desc)})`,
            });

            inputFileParameter.add(
              new Attribute(ParameterAttribute, {
                parameters: [
                  new LiteralExpression(
                    `Mandatory = ${vParam.required ? 'true' : 'false'}`
                  ),
                  new LiteralExpression(
                    `HelpMessage = "Input File for ${
                      cmdletParameter.name
                    } (${escapeString(desc || '.')})"`
                  ),
                ],
            ...
            if (length(vParam.alias) > 0) {
              inputFileParameter.add(
                new Attribute(Alias, {
                  parameters: vParam.alias.map((x) => '"' + x + '"'),
                })
              );
            }
            ...
            NewAddInfoAttribute(
              ...
              false,
              desc,
              (<NewVirtualProperty>vParam.origin).property.serializedName
            );
          ...
          const hasEnum =
            propertyType instanceof ArrayOf &&
            propertyType.elementType instanceof EnumImplementation;
          if (isEnum || hasEnum) {
            cmdletParameter.add(
              new Attribute(ArgumentCompleterAttribute, {
                parameters: [
                  `typeof(${
                    hasEnum
                      ? (<ArrayOf>propertyType).elementType.declaration
        ...
        const paramDictSchema =
          parameter.schema.type === SchemaType.Dictionary
            ? parameter.schema
            : parameter.schema.type === SchemaType.Object
              ? (<ObjectSchema>parameter.schema).parents?.immediate?.find(
                (s) => s.type === SchemaType.Dictionary
              )
              : undefined;
        if (paramDictSchema) {
          ...
          this.apProp = this.add(
            new Property(apPropName, System.Collections.Hashtable)
          );
          this.apProp.add(
            new Attribute(ParameterAttribute, {
              parameters: [
          ...
          this.bodyParameterInfo = {
            ...
            valueType:
              (<DictionarySchema>paramDictSchema).elementType.type ===
              SchemaType.Any
                ? System.Object
                : this.state.project.schemaDefinitionResolver.resolveTypeDeclaration(
                  (<DictionarySchema>paramDictSchema).elementType,
                  true,
                  this.state
                ),
          };
    ...
    if (this.isViaIdentity) {
      ...
      const idschema = values(this.state.project.model.schemas.objects).first(
        (each) => each.language.default.uid === 'universal-parameter-type'
      );
      const idtd =
        this.state.project.schemaDefinitionResolver.resolveTypeDeclaration(
      ...
      const idParam = this.add(
        new BackedProperty('InputObject', idtd, {
          description: 'Identity Parameter',
        })
      );
      const parameters = [
        new LiteralExpression('Mandatory = true'),
        new LiteralExpression('HelpMessage = "Identity Parameter"'),
        new LiteralExpression('ValueFromPipeline = true'),
      ];
      idParam.add(new Attribute(ParameterAttribute, { parameters }));
      idParam.add(
        new Attribute(CategoryAttribute, {
          parameters: [`${ParameterCategory}.Path`],
        })
      );
    }
    for (const vParam of values(vps.operation)) {
      ...
      if (vParam.type) {
        ...
        regularCmdletParameter = this.add(
          new BackedProperty(vParam.name, new ClassType('', vParam.type), {
        ...
        regularCmdletParameter =
          this.state.project.azure &&
          vParam.name === 'SubscriptionId' &&
          operation.details.csharp.verb.toLowerCase() === 'get'
            ? // special case for subscription id
            this.add(
              new BackedProperty(vParam.name, dotnet.StringArray, {
                metadata: {
            ...
            : // everything else
            this.add(
              new BackedProperty(vParam.name, propertyType, {
                metadata: {
                  parameterDefinition: origin.details.csharp.httpParameter,
                },
                description: vParam.description,
      ...
      const parameters = [
        ...
        new LiteralExpression(
          `HelpMessage = "${escapeString(vParam.description) || '.'}"`
        ),
      ];
      ...
      regularCmdletParameter.add(
        new Attribute(ParameterAttribute, { parameters })
      );
      if (!vParam.type && vParam.schema.type === SchemaType.Array) {
        regularCmdletParameter.add(
          new Attribute(AllowEmptyCollectionAttribute)
        );
      }
      ...
      if (httpParam) {
        ...
        const cat = operation.callGraph[0].parameters?.find(
          (param) =>
            !param.language.csharp?.constantValue &&
            param.language.csharp?.serializedName ===
              httpParam.language.csharp?.serializedName
        );
    ...
    const ifmatch = this.properties.find(
      (v) => v.name.toLowerCase() === 'ifmatch'
    );
    if (ifmatch) {
  ...
  private addDoNotExport(
    cmdletParameter: Property,
    vParam: NewVirtualParameter
  ) {
    if (vParam.hidden) {
      this.state.message({
  ...
  private NewAddClassAttributes(
    ...
    const cmdletAttribParams: Array<ExpressionOrLiteral> = [
      category[operation.details.csharp.verb]
        ? verbEnum(
          category[operation.details.csharp.verb],
          operation.details.csharp.verb
        )
        : `"${operation.details.csharp.verb}"`,
    ...
    if (operation.details.csharp.hidden) {
      ...
      const cmdletName = `${operation.details.csharp.verb}-${noun}${
        operation.details.csharp.name ? `_${operation.details.csharp.name}` : ''
      }`;
      this.state.message({
        Channel: Channel.Debug,
        Text: `[DIRECTIVE] Applied 'hide' directive to ${cmdletName}. Added attribute ${InternalExportAttribute.declaration} to cmdlet.`,
      });
    }

    this.add(
      new Attribute(CmdletAttribute, { parameters: cmdletAttribParams })
    );
    ...
    if (length(operation.details.csharp.alias) > 0) {
      this.add(
        new Attribute(Alias, {
          parameters: operation.details.csharp.alias.map(
            (x: string) => '"' + x + '"'
          ),
        })
      );
    ...
    for (const httpOperation of values(operation.callGraph)) {
      ...
      const v =
        ...
        httpOperation.responses[0] instanceof SchemaResponse;
      // Add this for binary response in m4
      for (const binary of values(httpOperation.responses).selectNonNullable(
        (each) => (<BinaryResponse>each).binary
      )) {
        if (binary) {
      ...
      for (const schema of values(httpOperation.responses).selectNonNullable(
        (each) => (<SchemaResponse>each).schema
      )) {
        const props = NewGetAllProperties(schema);

        // does the target type just wrap a single output?
        const resultSchema =
          length(props) !== 1 ? <NewSchema>schema : <NewSchema>props[0].schema;

        // make sure return type for boolean stays boolean!
        if (
          resultSchema.type === SchemaType.Boolean ||
          (resultSchema.type === SchemaType.Choice &&
            (<any>resultSchema).choiceType.type === SchemaType.Boolean &&
            (<ChoiceSchema>resultSchema).choices.length === 1) ||
          (resultSchema.type === SchemaType.SealedChoice &&
            (<any>resultSchema).choiceType.type === SchemaType.Boolean &&
            (<SealedChoiceSchema>resultSchema).choices.length === 1)
        ) {
          outputTypes.add(`typeof(${dotnet.Bool})`);
        } else {
          ...
          if (
            typeDeclaration.declaration === System.IO.Stream.declaration ||
            typeDeclaration.declaration === dotnet.Binary.declaration
          ) {
            // if this is a stream, skip the output type.
            this.hasStreamOutput = true;
            shouldAddPassThru = true;
            ...
            if (typeDeclaration instanceof ArrayOf) {
              ...
              pageableInfo &&
              pageableInfo.responseType === 'pageable'
            ) {
              if (
                typeDeclaration === undefined ||
                (<ObjectSchema>typeDeclaration.schema).properties?.find(
                  (p) => p.serializedName === pageableInfo.itemName
                ) === undefined
              ) {
    ...
    shouldAddPassThru =
      shouldAddPassThru ||
      values(operation.callGraph)
        .selectMany((httpOperation) =>
          values(
            (httpOperation.responses || []).concat(
              httpOperation.exceptions || []
            )
          )
        )
    ...
    if (operation.details.csharp.breakingChange) {
      ...
      if (breakingChange.cmdlet) {
        ...
        if (breakingChange.cmdlet.deprecateByVersion) {
          ...
          if (breakingChange.cmdlet.changeInEfectByDate)
            parameters.push(`"${breakingChange.cmdlet.changeInEfectByDate}"`);
        }
        if (breakingChange.cmdlet.replacement)
          parameters.push(
            `ReplacementCmdletName="${breakingChange.cmdlet.replacement}"`
          );
        if (breakingChange.cmdlet.changeDescription)
          parameters.push(
            `ChangeDescription="${breakingChange.cmdlet.changeDescription}"`
          );

        this.add(
          new Attribute(ClientRuntime.CmdletBreakingChangeAttribute, {
            parameters: parameters,
          })
        );
      }
      if (breakingChange.variant) {
        ...
        if (breakingChange.variant.deprecateByVersion) {
          ...
          if (breakingChange.variant.changeInEfectByDate)
            parameters.push(`"${breakingChange.variant.changeInEfectByDate}"`);
        }
        if (breakingChange.variant.changeDescription)
          parameters.push(
            `ChangeDescription="${breakingChange.variant.changeDescription}"`
          );

        this.add(
          new Attribute(ClientRuntime.ParameterSetBreakingChangeAttribute, {
            parameters: parameters,
          })
        );
      }
      if (breakingChange.output) {
        ...
        if (breakingChange.output.deprecatedCmdLetOutputType) {
          parameters.push(
            `"${breakingChange.output.deprecatedCmdLetOutputType}"`
          );
        } else {
          parameters.push(
            `"${outputTypes
              .values()
              .next()
              .value.replace(/typeof\((.*)\)/, '$1')}"`
          );
        }
        if (breakingChange.output.deprecateByVersion) {
          parameters.push(`"${breakingChange.output.deprecateByVersion}"`);
          if (breakingChange.output.changeInEfectByDate)
            parameters.push(`"${breakingChange.output.changeInEfectByDate}"`);
        }
        if (breakingChange.output.replacement)
          parameters.push(
            `ReplacementCmdletOutputType="${breakingChange.output.replacement}"`
          );
        if (breakingChange.output.deprecatedOutputProperties) {
          const properties: Array<string> = Object.assign(
            [],
            breakingChange.output.deprecatedOutputProperties
          );
          properties.forEach(
            (element, index) => (properties[index] = '"' + element + '"')
          );
          parameters.push(
            `DeprecatedOutputProperties=new string[] {${properties.join(',')}}`
          );
        }
        if (breakingChange.output.newOutputProperties) {
          ...
          parameters.push(
            `NewOutputProperties=new string[] {${properties.join(',')}}`
          );
        }
        if (breakingChange.output.changeDescription)
          parameters.push(
            `ChangeDescription="${breakingChange.output.changeDescription}"`
          );

        this.add(
          new Attribute(ClientRuntime.OutputBreakingChangeAttribute, {
            parameters: parameters,
          })
        );
      }
    ...
    if (operation.details.csharp.previewMessage) {
      this.add(
        new Attribute(ClientRuntime.PreviewMessageAttribute, {
          parameters: [`"${operation.details.csharp.previewMessage}"`],
        })
      );
    }
    this.add(
      new Attribute(OutputTypeAttribute, { parameters: [...outputTypes] })
    );
    if (shouldAddPassThru) {
      const passThru = this.add(
        new Property('PassThru', SwitchParameter, {
          description:
      ...
      passThru.add(
        new Attribute(ParameterAttribute, {
          parameters: [
            'Mandatory = false',
            'HelpMessage = "Returns true when the command succeeds"',
          ],
        })
      );
      passThru.add(
    ...
    this.add(
      new Attribute(DescriptionAttribute, {
        parameters: [new StringExpression(this.description)],
      })
    );
    ...
    if (operation.details.default.externalDocs) {
      this.add(
        new Attribute(ExternalDocsAttribute, {
          parameters: [
            `${new StringExpression(
              this.operation.details.default.externalDocs?.url ?? ''
            )}`,
            `${new StringExpression(
              this.operation.details.default.externalDocs?.description ?? ''
            )}`,
          ],
        })
      );
    ...
    if (
      ...
      operation.extensions['x-ms-metadata'] &&
      operation.extensions['x-ms-metadata'].profiles
    ) {
      const profileNames = Object.keys(
        operation.extensions && operation.extensions['x-ms-metadata'].profiles
      );
      // wrap profile names
      ...
      this.add(
        new Attribute(ProfileAttribute, { parameters: [...profileNames] })
      );
    }
    ...
    this.operation.callGraph.forEach((operationInfo) => {
      ...
      operationInfo.requests?.forEach((request) => {
        this.add(
          new Attribute(HttpPathAttribute, {
            parameters: [
